...
근거 기사까지. 한국어와 영어 양쪽으로.*

제22대 국회의원 296명과 정당 8개. 뉴스 본문에서 추론한 관계 엣지 637개 — 갈등 362개,
우호 275개 — 그중 607개가 기사 단위로 열어 볼 수 있는 근거 로그를 달고 있습니다. 매일 도는
...
![테스트](https://img.shields.io/badge/tests-162-22d3ee)
...
보도되고, 처음 몇 주의 데이터도 그랬습니다 — 2026-09-11 기준 갈등 엣지 107개에 우호
엣지 32개. 그 차이는 지금 362개 대 275개로 좁아졌습니다. 가장 먼저 의심한 것은
2026-09-13 에 넣은 창 상한(쌍마다 채점할 문장 창의 최대 개수)이었는데, 같은 기사 22건에
상한을 켜고 꺼서 돌려 보니 **판정이 하나도 바뀌지 않았습니다.** 쌍 하나가 판정에 쓰는
창은 중앙값 1개, 많아야 2개여서 상한에 닿지도 않습니다
(`backend/scripts/compare_window_cap.py`). 바뀐 것은 읽는 양입니다. 예전에는 러너 한 대가
그날 분량의 60~75%까지 갔는데 지금은 여섯 대가 전부 읽습니다. 다만 "국회가 원래
갈등투성이다" 와 "갈등이 주로 기사가 된다" 는 여전히 가려내지 못합니다. 볼 수 있는 것이
보도뿐이기 때문입니다. 그래서 이 치우침을 세탁하지 않고 드러내는 쪽으로 설계돼
...
| **두 시간 창을 따로 표시** | 화제성은 7일 롤링, 관계는 수집을 시작한 2026-08-30 부터 누적입니다. 섞으면 화면에 제 합과 안 맞는 숫자가 나가므로, 각 칸에 어느 창인지 밝히는 `?` 를 답니다. 화면의 모든 날짜는 한국시간이고, 매일 밤 회차가 데이터에 찍는 기준일도 한국시간입니다. |
...
대표적인 한 회차(2026-09-18): 기사 1,136건 수집, 러너 여섯 대가 300건 분석, 243건 저장,
112쌍이 엣지로 승격, 의원 256명에 대해 화제성 1,156건 기록 — 처음부터 끝까지 77분.
...
- **한 진영의 물량은 교차 검증이 아닙니다.** 한쪽 언론 스무 건은 하나의 편집 판단이
  ...
  그 상한을 화면에서 점선으로 드러냅니다. 2026-09-19 기준 637개 중 실선은 72개뿐인데,
...
# 이 쌍은 2026-09-19 기준 기사 21건에서 사건 20건, 세 진영 모두에서 잡혀 있습니다.
...
- **대부분의 엣지는 이 방법이 내세우는 기준을 아직 넘지 못했습니다.** 2026-09-19 기준
  관계 엣지 637개 중 **진영 교차로 확인된 것은 72개**, 535개는 한 진영 보도에만 기대고
  있으며, **30개는 근거 기록 자체가 없습니다** — 근거 테이블이 생기기 전에 만들어져
  `backfill_edge_observations.py` 를 기다리는 것들이고, 그중 둘은 기사가 없는 예시
  데이터입니다. 별개로 **431개는 사건 하나에만 기대고 있습니다.** 아카이브가 아직 어려
  `RELATION_MIN_CLUSTERS` 가 1이기 때문입니다. 화면은 이것들을 모두 점선으로 그리지만,
  정직한 요약은 진영 교차 검증이 낮은 숫자를 보고하고 있는 작동 중인 장치라는 것입니다.
- **아카이브가 21일치입니다.** "누적" 은 2026-08-30 부터라는 뜻입니다. 시간 감쇠, 사건
