...
이 프로젝트는 **표결이 아니라 언론 보도**를 근거로 관계를 만듭니다. 그러면 언론의 선택 편향이 그대로 관계도에 들어옵니다. 실제로 r/politicalscience 에서 "편향 보도와 어그로성 기사 때문에 데이터를 믿기 어렵다" 는 지적을 받았고, 그에 답하려고 정치학·커뮤니케이션학·전산언어학 논문을 조사해 보정 단계를 넣었습니다.

조사 내용과 설계는 [docs/MEDIA_BIAS_RESEARCH.md](docs/MEDIA_BIAS_RESEARCH.md), 적용 결과와 실측값은 [docs/ALGORITHM_REPORT.md](docs/ALGORITHM_REPORT.md) 에 있습니다.

### 어떤 편향을 무엇으로 막았나

| 편향 | 이 프로젝트에서 어떻게 나타났나 | 보정 | 근거 |
| :--- | :--- | :--- | :--- |
| **덮어쓰기로 인한 근거 소실** | 같은 쌍의 마지막 기사가 앞의 전부를 지워, 기사 수·언론사·시간이 남지 않았습니다 | **근거 로그**. 기사 하나의 판정 하나를 한 행으로 남기고, 엣지는 그 집계 결과만 담습니다 | — |
| **논조 편향** (tonality bias) | 어느 진영 매체가 보도했는지 알 수 없어 언론사 성향을 뺄 수 없었습니다 | 관측마다 **언론사와 진영**을 저장합니다 | Eberl 외 2017, 최창식·임영호 2021 |
| **게이트키핑 · 공격 선택** | 한 진영만 쓴 갈등도 확정 사실처럼 그려졌습니다 | **진영 교차 검증**. 보수·중도·진보가 각자 독립 보도했을 때만 신뢰도가 오르고, 한 진영은 상한(0.7)을 못 넘습니다 | Mullainathan & Shleifer 2005, Gentzkow & Shapiro 2006, Budak 외 2016, 박영흠 2024 |
| **전재로 인한 물량 착시** | 통신사 기사가 여러 URL 로 들어와 각각 한 표로 세어졌습니다 | **사건 단위 중복 제거**. 본문 SimHash 로 묶어 한 표로 셉니다. 여러 진영에 걸친 전재는 통신사 판단이므로 중도로 접습니다 | 연합뉴스의 포털 송고 비중 |
| **프레이밍 · 발언 선택** | 기자가 붙인 대립 구도와 정치인이 실제로 한 말이 같은 무게였습니다 | **발화 주체 귀속**. 방향형으로 묻고 직접 인용(1.0)·간접 인용(0.7)·기자 서술(0.3)을 구분합니다. 추측성 표현은 절반으로 깎습니다 | Entman 1993, Recasens 외 2013, 유재광·오경수 2012 |
| **자극적 문장의 과대 대표** | 기사당 최고점 하나가 기사 전체를 대표했습니다 | **상위 3개 창의 가중 평균**으로 바꿨습니다 | — |
| **나열 기사 희석** | 의원 10명을 나열한 기사에서 뽑은 쌍이 단독 기사와 같은 무게였습니다 | 개인당 **1/√n** 로 나눕니다 | — |
| **동적 편향** | 편향과 관계는 시기에 따라 변하는데 누적값만 있었습니다 | **반감기 45일**. 최근 논조와 누적 이력을 따로 보관합니다 | Kim·Lelkes·McCrain 2022, 이신행 2024 |
| **근거 없는 데이터 혼입** | 임포터가 넣던 손으로 적은 관계 5건이 실제 관측과 구분되지 않았습니다 (척도마저 0~100 대 0~1) | **생성 중단**. 기존 데이터는 감사 스크립트가 목록으로 보여 줍니다 | — |

### 보정이 실제로 무엇을 바꿨나

작업 중 직접 측정한 값입니다. 손으로 쓴 예시 문장 기준이라 벤치마크가 아니라 **설계를 바꾼 진단**입니다.

**대칭 가설이 갈등을 버리고 있었습니다.** "A와 B는 서로 적대적인 관계이다" 는 *상호* 상태를 주장하는데, 한쪽이 비판한 문장은 이를 함의하지 않습니다.

| 전제: "나경원 의원은 이재명 대표를 겨냥해 비판했다" | 함의 확률 |
| :--- | ---: |
| 기존 대칭 가설 | 0.607 ← 임계값 0.65 미달로 **버려짐** |
| 방향형 "나경원은 이재명을 비판했다" | 0.956 |
| 역방향 "이재명은 나경원을 비판했다" | 0.093 |

**공동 언급이 관계로 둔갑하던 것을 막았습니다.** 의원 5명이 나오는 기사 한 건에서:

| 규칙 | 만들어진 관계 | 그중 오탐 |
| :--- | ---: | ---: |
| 창 안에 두 이름이 있으면 인정 | 10 | 6 |
| 셋 이상일 땐 같은 문장을 요구 | 4 | 0 |

**전재로 교차 검증을 통과할 수 없습니다.**
...
**한 진영은 물량으로 신뢰를 살 수 없습니다.** 보수지 12건이 0.70(상한)인데, 보수 3건 + 진보 3건은 0.85 입니다.

### 아직 보정하지 못한 것

- **언론사별 논조 기준선**: 조선일보가 민주당 의원을 비판한 기사와 한겨레가 같은 의원을 비판한 기사는 정보량이 다릅니다. 이를 빼려면 언론사×정당 셀마다 표본이 수십 건은 필요한데 아직 없습니다.
- **부정성 선택 편향 역가중**: 협력은 보도되지 않으므로 우호 관계가 구조적으로 부족합니다. 뉴스 밖 증거인 국회 공동발의로 보완하려 했으나 API 키가 없어 자료가 비어 있습니다.
- **어그로(클릭베이트) 할인**: 나열 기사 가중만 넣었고, 제목-본문 불일치 분류기는 아직입니다.
- **수집원 다변화**: 언론사 진영은 통제하지만 포털 한 곳의 편집 선택은 통제하지 못합니다.

**가장 큰 한계**: 자동 추출한 관계에 대해 **사람이 검증한 표본이 아직 없습니다.** 정밀도·재현율 수치가 없으므로, 관계 데이터는 아직 분석 결과가 아니라 예시로 보아야 합니다. 층화 표집과 Krippendorff's alpha 채점 도구는 준비돼 있습니다(아래 운영 절 참조).

### 근거를 직접 확인하려면

**화면에서**: 관계선을 누르면 아래 패널에 그 관계의 근거가 펼쳐집니다. 사건 수, 진영별 사건 수, 신뢰도, 그리고 **근거 기사 목록이 원문 링크와 함께** 나옵니다. 전재 여러 건이 한 사건으로 묶인 경우 그 사실도 표시합니다.
