...
class Neo4jImporter:
    ...
    def get_member_relationships(self, member_id, max_depth=2):
        ...
        with self.driver.session() as session:
            ...
            LIMIT 100
            """
            result = session.run(query, member_id=member_id, max_depth=max_depth)
...
def main():
    """메인 실행 함수"""
    # Neo4j 연결 (Docker 기본 설정)
    importer = Neo4jImporter(
        uri="bolt://localhost:7687",
        user="neo4j",
        password="patrol-alpine-thomas-nepal-deposit-3273"
    )
    ...
    try:
        ...
        if os.path.exists(json_file):
            ...
            # 테스트 검색
            print("\n=== 테스트 검색 ===")
            members = importer.search_member("김")
            print(f"'김'이 포함된 의원 {len(members)}명:")
            for member in members[:5]:  # 처음 5명만 출력
                print(f"  {member['name']} ({member['party']}, {member['region']})")
