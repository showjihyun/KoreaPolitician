...
Render 무료 웹 서비스는 **만료가 없습니다.** 월 750 instance-hour 안에서
기간 제한 없이 무료이고, 한도를 넘겨도 삭제가 아니라 다음 달까지 정지될
뿐이며 `onrender.com` 주소도 유지됩니다. (30일 만료는 Render 의 무료
PostgreSQL 얘기이고, 우리는 Supabase 를 쓰므로 해당 없습니다.)

슬립을 끄지 않고 그대로 둡니다.

| | 월 사용 | 위험 |
| --- | ---: | --- |
| **슬립 유지 (채택)** | 실제 트래픽 시간만 | 없음. 한도 근처도 가지 않음 |
| 크론으로 상시 가동 | 744시간(31일) | 한도 750시간까지 **여유 6시간**. 초과 시 그 달 정지 |

상시 가동은 여유가 너무 빠듯해 채택하지 않았습니다. 월말에 한도를 넘기면
1분 지연이 아니라 서비스가 아예 뜨지 않습니다. 그래서 keep-alive 워크플로를
일부러 두지 않았습니다.

대가는 **15분 유휴 후 첫 요청이 약 1분** 걸린다는 것입니다. 이로 인한
부작용은 두 군데에서 처리했습니다.

- **크롤러**: 새벽 4시에 도는데 그때 서버는 확실히 자고 있습니다. 워밍업
  없이 바로 `POST /api/edge` 를 치면 첫 요청들이 타임아웃으로 버려져 관계가
  조용히 유실됩니다. `news_crawler_pipeline.wake_api()` 가 `/health` 를
  최대 3분간 재시도해 깨운 뒤 파이프라인을 시작합니다.
- **인메모리 그래프**: SNS 크롤러가 DB 에 직접 쓴 엣지는 API 메모리에 즉시
  반영되지 않습니다. 다만 이 구성에서는 서비스가 자주 재기동되고 그때마다
  `load_from_db()` 로 다시 읽으므로 실질적으로 해소됩니다. 유료로 올려
  상시 가동하게 되면 주기적 재적재가 필요합니다.
